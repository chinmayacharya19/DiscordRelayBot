...
function sendMessage(
    ...
    if (options && options.richEmbed) {
        ...
        let str = "**New Copied Message**\n*Authored in: **" + message.guild.name + "/"
        if (!messageContents.includes(str)) {
            // Add title if requested.
            ...
            if (options && options.includeSource) {
                destinationMessage += `*Authored in: **${message.guild.name
                    }/${(message.channel as TextChannel).name}***\n`;
            }
        }
        ...
        // let tc = message.channel as discord.TextChannel;
